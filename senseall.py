...
def connectRandom():
    threading.Timer(120, connectRandom).start()
	...
	if not os.path.exists('logs/logall/%s'%(address)):
	    os.mkdir('logs/logall/%s'%(address))
	with open('logs/logall/%s/%s.csv'%(address, gw),'a') as f:
